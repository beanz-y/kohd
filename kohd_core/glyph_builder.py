...
from .trace_router import calculate_trace_path # Import the new router function
import math
...
class KohdGlyphBuilder:
    def __init__(self, node_radius: float, get_ring_radius_method: callable): # Accept config
        self.rules = {
        ...
        # Store canvas-related constants
    ...
    def _find_null_modifier_placement_node(self) -> str | None:
        ...
        for corner_node_name in preferred_placement_order:
            if corner_node_name not in self.current_word_used_node_names and \
               corner_node_name not in bounding_box_nodes: 
