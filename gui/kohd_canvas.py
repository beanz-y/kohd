    ...
    def paintEvent(self, event):
        ...
        for element in self.glyph_elements_to_draw: 
            if element['type'] == 'trace':
                ...
                if from_name in nodes_render_data and to_name in nodes_render_data and not nodes_render_data[from_name].get('is_null_modifier_location') and not nodes_render_data[to_name].get('is_null_modifier_location'):
        ...
        if null_modifier_info: 
            ...
            painter.drawEllipse(mod_center, self.node_radius, self.node_radius)
            ...
            painter.drawLine(QPointF(cx - offset, cy - offset), QPointF(cx + offset, cy + offset)); painter.drawLine(QPointF(cx - offset, cy + offset), QPointF(cx + offset, cy - offset))
            mno_node_data = nodes_render_data.get('MNO')
            if mno_node_data:
                mno_center = mno_node_data['coords']
                if mod_center != mno_center:
                    pointer_start_on_mod_circumference = self._calculate_connection_point(mod_center, mno_center, mod_center, 0)
                    line_mod_to_mno = QLineF(mod_center, mno_center); dist_mod_to_mno_center = line_mod_to_mno.length()
                    if dist_mod_to_mno_center > 1e-3:
                        pointer_line_visual_length = dist_mod_to_mno_center * 0.40
                        pointer_end_center = line_mod_to_mno.pointAt(pointer_line_visual_length / dist_mod_to_mno_center) # Center of small circle
                        
                        # Calculate where the line segment should visually end (on circumference of small circle)
                        ...
                        visual_line_end = pointer_end_center # Default if too short to adjust
                        if len_to_small_center > self.null_modifier_pointer_line_radius:
                        ...
                        if QLineF(pointer_start_on_mod_circumference, visual_line_end).length() > 1e-2 : # Draw line only if it has some length
                        ...
                        painter.setBrush(Qt.BrushStyle.NoBrush) # Hollow circle
                        painter.setPen(QPen(QColor(Qt.GlobalColor.darkGray), self.ring_pen_width * 0.6))
                        painter.drawEllipse(pointer_end_center, self.null_modifier_pointer_line_radius, self.null_modifier_pointer_line_radius)
