...
from PyQt6.QtGui import QPainter, QColor, QPen, QBrush, QFont, QPalette, QPainterPath, QTransform
from PyQt6.QtCore import Qt, QRectF, QPointF, QLineF

from kohd_core.kohd_rules import NODE_POSITIONS, RING_NODE_INSET_FACTOR, SUBNODE_RADIUS

MAX_RINGS_TO_DRAW = 2
# Preferred angles, starting from left, then diagonals, then bottom/top
PREFERRED_CHARGE_ANGLES_DEG = [180, 225, 135, 270, 90, 315, 45, 0] 
# Preferred angles for ground, prioritizing bottom, then sides, then diagonals, then top
PREFERRED_GROUND_TRACE_ANGLES_DEG = [270, 225, 315, 180, 0, 135, 45, 90] 
MIN_ANGLE_SEPARATION_DEG = 30 # Minimum separation

class KohdCanvasWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        # ... (rest of __init__ remains the same, ensure user's preferred padding is used for subnodes) ...
        ...
        self.subnode_trace_start_to_first_dot_center_padding = self.subnode_dot_radius * 4.0 # As per user's preference
    ...
    # ... (update_display_data, _get_radius_for_specific_ring_level, _calculate_connection_point_at_angle, _calculate_connection_point, _draw_subnodes_on_line are UNCHANGED) ...
    ...
    def _calculate_connection_point(self, line_origin_center: QPointF, line_target_center: QPointF, connection_node_center: QPointF, connection_ring_level: int) -> QPointF:
        ...
        if line_length < 1e-3 or line_length < radius_to_connect + 1e-3 : return connection_node_center # Added check for very small length
    ...
    def paintEvent(self, event):
        ...
        # --- 1. Prepare Node Data & Collect Actual Trace Angles ---
        nodes_render_data = {}
        ...
        # Collect angles from actual 'trace' elements (not indicators or ground trace yet)
        for element in self.glyph_elements_to_draw:
            if element['type'] == 'trace':
                ...
                if from_name in nodes_render_data and to_name in nodes_render_data:
                    p1 = nodes_render_data[from_name]['coords']; p2 = nodes_render_data[to_name]['coords']
                    line = QLineF(p1, p2)
                    if line.length() > 1e-3: # Avoid zero-length line issues
                        node_actual_trace_angles[from_name].append(line.angle())
                        node_actual_trace_angles[to_name].append(QLineF(p2,p1).angle())
        
        # --- Determine Indicator and Ground Trace Angles ---
        ...
        if charge_indicator_element and charge_indicator_element['node_name'] in nodes_render_data:
            node_name = charge_indicator_element['node_name']
            chosen_charge_angle_deg = self._find_clear_angle_deg(
                node_actual_trace_angles.get(node_name, []),
                PREFERRED_CHARGE_ANGLES_DEG,
                MIN_ANGLE_SEPARATION_DEG
            )

        chosen_ground_trace_angle_deg = 270 # Default downwards
        if trace_to_ground_element and trace_to_ground_element['from_node_name'] in nodes_render_data:
            from_node_name = trace_to_ground_element['from_node_name']
            temp_existing_angles = list(node_actual_trace_angles.get(from_node_name, []))
            if charge_indicator_element and charge_indicator_element['node_name'] == from_node_name and chosen_charge_angle_deg is not None:
                temp_existing_angles.append(chosen_charge_angle_deg) # Consider charge angle if same node
            
            chosen_ground_trace_angle_deg = self._find_clear_angle_deg(
                temp_existing_angles,
                PREFERRED_GROUND_TRACE_ANGLES_DEG,
                MIN_ANGLE_SEPARATION_DEG
            )

        # --- Draw Order ---
        # 1. Fills, 2. Outlines, 3. Rings (same as before)
        # ... (code for drawing fills, outlines, rings is identical to previous version) ...
        ...
        # --- 4. Trace Lines (Actual Traces first, then Trace To Ground) ---
        last_trace_to_ground_visual_endpoint = None # Reset for this paint pass
        # Draw 'trace' elements
        for element in self.glyph_elements_to_draw:
            if element['type'] == 'trace':
                # ... (same logic as before for drawing 'trace' type elements) ...
                ...
                if from_node_name in nodes_render_data and to_node_name in nodes_render_data:
                    from_node_center = nodes_render_data[from_node_name]['coords']; to_node_center = nodes_render_data[to_node_name]['coords']
                    connect_from_level = element.get('connect_from_ring_level', 0); connect_to_level = element.get('connect_to_ring_level', 0)
                    ...
                    self._draw_subnodes_on_line(painter, visual_start_point, visual_end_point, element.get('subnodes_on_trace', []), connect_from_level)
        
        # Draw 'trace_to_ground' element using its determined clear angle
        if trace_to_ground_element and trace_to_ground_element['from_node_name'] in nodes_render_data:
            from_node_name = trace_to_ground_element['from_node_name']
            from_node_center = nodes_render_data[from_node_name]['coords']
            connect_from_ring_level = trace_to_ground_element.get('connect_from_ring_level', 0)
            subnodes_list = trace_to_ground_element.get('subnodes_on_trace', [])
            ...
            # ... (same dynamic length calculation for ground_trace_visual_length as before, using effective_start_pad) ...
            effective_start_pad = self.subnode_trace_start_to_first_dot_center_padding
            if connect_from_ring_level > 0:
            ...
            if num_final_dots > 0:
                ...
                if num_final_dots > 0:
                    required_subnode_span += (num_final_dots -1) * self.subnode_intra_group_center_to_center_spacing if num_final_dots > 1 else 0
                    required_subnode_span += self.subnode_dot_radius 
            ...
            visual_end_point = QPointF(
                visual_start_point.x() + ground_trace_visual_length * math.cos(rad_angle),
                visual_start_point.y() - ground_trace_visual_length * math.sin(rad_angle)
            )
            last_trace_to_ground_visual_endpoint = visual_end_point # For ground symbol
        ...
        if charge_indicator_element and chosen_charge_angle_deg is not None:
            # ... (Draw charge indicator using chosen_charge_angle_deg - logic mostly same as before)
            node_name = charge_indicator_element['node_name']
            node_center = nodes_render_data[node_name]['coords']
            start_on_circumference = self._calculate_connection_point_at_angle(node_center, 0, chosen_charge_angle_deg)
            rad_clear_angle = math.radians(chosen_charge_angle_deg)
            line_to_symbol_end_x = start_on_circumference.x() + self.indicator_symbol_base_size * 0.5 * math.cos(rad_clear_angle)
            line_to_symbol_end_y = start_on_circumference.y() - self.indicator_symbol_base_size * 0.5 * math.sin(rad_clear_angle)
            line_to_zigzag_end = QPointF(line_to_symbol_end_x, line_to_symbol_end_y)
            painter.setPen(indicator_pen); painter.drawLine(start_on_circumference, line_to_zigzag_end)
            path = QPainterPath(); path.moveTo(line_to_zigzag_end)
            ...
            for i in range(1, num_zig_points):
                dist_along_angle = (i / (num_zig_points -1)) * zigzag_width_total
                perp_offset_val = (zigzag_height / 2) * ((i % 2) * 2 - 1)
                ...
                final_x = px + perp_offset_val * math.sin(rad_clear_angle); final_y = py + perp_offset_val * math.cos(rad_clear_angle) # Swapped sin/cos for perp vector
        ...
        if ground_indicator_element and last_trace_to_ground_visual_endpoint is not None:
            # ... (Draw ground indicator using last_trace_to_ground_visual_endpoint and chosen_ground_trace_angle_deg for orientation - logic same as before)
            indicator_attach_point = last_trace_to_ground_visual_endpoint
            painter.setPen(indicator_pen); bar_width = self.indicator_symbol_base_size 
            painter.save(); painter.translate(indicator_attach_point) 
            # Note: last_trace_to_ground_angle_deg was from start to end of ground trace.
            # For symbol drawing, this is the angle of the line "coming into" the symbol.
            # PDF symbol has horizontal bar for vertical incoming line. So bar is perpendicular.
            perp_angle_rad = math.radians(chosen_ground_trace_angle_deg - 90)
            p1x = -bar_width/2 * math.cos(perp_angle_rad); p1y =  bar_width/2 * math.sin(perp_angle_rad) 
            p2x =  bar_width/2 * math.cos(perp_angle_rad); p2y = -bar_width/2 * math.sin(perp_angle_rad)
            painter.drawLine(QPointF(p1x, p1y), QPointF(p2x, p2y))
            leg_len = bar_width * 0.4
            # Legs point "back" along the trace, so use angle + 180
            leg_dx = leg_len * math.cos(math.radians(chosen_ground_trace_angle_deg + 180)) 
            leg_dy = leg_len * -math.sin(math.radians(chosen_ground_trace_angle_deg + 180))
            painter.drawLine(QPointF(p1x, p1y), QPointF(p1x + leg_dx, p1y + leg_dy)); painter.drawLine(QPointF(p2x, p2y), QPointF(p2x + leg_dx, p2y + leg_dy))
            gap = self.indicator_symbol_base_size * 0.3
            # Offset for second part of symbol along the trace's direction
            gap_dx_main = gap * math.cos(math.radians(chosen_ground_trace_angle_deg)); gap_dy_main = gap * -math.sin(math.radians(chosen_ground_trace_angle_deg))
            second_symbol_center_x = gap_dx_main; second_symbol_center_y = gap_dy_main
            small_bar_w = bar_width * 0.7; small_leg_h = bar_width * 0.3
            sp1x = second_symbol_center_x - small_bar_w/2 * math.cos(perp_angle_rad); sp1y = second_symbol_center_y + small_bar_w/2 * math.sin(perp_angle_rad)
            sp2x = second_symbol_center_x + small_bar_w/2 * math.cos(perp_angle_rad); sp2y = second_symbol_center_y - small_bar_w/2 * math.sin(perp_angle_rad)
            # Legs point "forward" along the trace for the smaller bracket
            ...
            painter.drawLine(QPointF(sp1x, sp1y), QPointF(sp1x + s_leg_dx, sp1y + s_leg_dy)); painter.drawLine(QPointF(sp2x, sp2y), QPointF(sp2x + s_leg_dx, sp2y + s_leg_dy))
            mid_x_offset_from_second_center = 0; mid_y_offset_from_second_center = 0 # Center of the non-existent bar
            mid_line_len = small_leg_h * 1.4 
            # Line along the trace direction
            m_start_dx = -mid_line_len/2 * math.cos(math.radians(chosen_ground_trace_angle_deg)); m_start_dy = -mid_line_len/2 * -math.sin(math.radians(chosen_ground_trace_angle_deg))
            m_end_dx = mid_line_len/2 * math.cos(math.radians(chosen_ground_trace_angle_deg)); m_end_dy = mid_line_len/2 * -math.sin(math.radians(chosen_ground_trace_angle_deg))
            painter.drawLine(QPointF(second_symbol_center_x + m_start_dx, second_symbol_center_y + m_start_dy), QPointF(second_symbol_center_x + m_end_dx, second_symbol_center_y + m_end_dy))
            painter.restore()
